...
<Info>
  A scenario's manifest exposes a `time_basis` block with `date_semantics`, `path_mode`, and `path_mode_description` so agents can reason about the clock correctly. See [`GET /scenarios/{id}/manifest`](/api-reference/scenarios#manifest).
</Info>
...
| --- | --- | --- |
| `as_of` | Point-in-time cutoff, return only data visible on or before this date. | News, events, snapshots, estimates, metrics snapshot, interest-rate snapshot |
