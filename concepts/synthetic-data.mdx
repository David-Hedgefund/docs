...
2. **Company reference identity.** Display metadata (name, sector, industry, business description, employee count, market-cap tier, instrument type) is sourced from **yfinance** and cached, so `AAPL` reads as *Apple, Consumer Electronics* and `SPY` reads as *an S&P 500 ETF*.

<Warning>
  Real symbols and real names do **not** mean real data. The prices, fundamentals, news, filings, ownership, and events behind every symbol are synthetic scenario data. A symbol that yfinance has no data for is left `Unclassified` rather than assigned a fabricated sector.
</Warning>
...
Every scenario ships a [validation report](/api-reference/scenarios#validation-report) that checks these properties, plus a per-ticker [dataset-quality report](/api-reference/scenarios#dataset-quality).
