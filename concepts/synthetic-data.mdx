...
2. **Company reference identity.** Display metadata (name, sector, industry, business description, employee count, market-cap tier, instrument type) is sourced from **yfinance** and cached in the `company_reference` table, so `AAPL` reads as *Apple, Consumer Electronics* and `SPY` reads as *an S&P 500 ETF*.
