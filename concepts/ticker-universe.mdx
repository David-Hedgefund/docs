...
Display identity (name, sector, industry, business description, employees, market-cap tier, reference price, and instrument type) is sourced from **yfinance** and cached in the `company_reference` table, not hand-curated. Instrument type (ETF, warrant, unit, right, preferred share, and similar) is detected from the symbol and company name.
