...
## Company reference identity

Display identity (name, sector, industry, business description, employees, market-cap tier, reference price, and instrument type) is sourced from **yfinance** and cached, not hand-curated. Major companies and ETFs have explicit overrides; the long tail falls back to name-based classification. A symbol with no yfinance data is left `Unclassified` rather than assigned a fabricated sector.
