...
Beyond the symbol, each company's display identity (name, sector, industry, exchange, and instrument type) reflects the real-world company, so listings read naturally: `AAPL` is Apple, `SPY` is an S&P 500 ETF. Only the prices, fundamentals, and other data behind each symbol are synthetic.
