...
| `filing_type` | string | all | Filter by form, e.g. `10-Q`, `8-K`. |
| `start_date` / `end_date` | date | - | Filing-date window. |
...
| `scenario_id` | string | - | **Required.** |
| `ticker` | string | all | Restrict to one ticker. |
| `filing_type` | string | all | Filter by form. |
| `item` | string | all | Filter by item code, e.g. `Item-2`. |
| `year` | integer | all | Filter by filing year. |
| `quarter` | integer | all | Filter by quarter (1–4). |
| `start_date` / `end_date` | date | - | Filing-date window. |
