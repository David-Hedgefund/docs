...
| `ticker` | string | Symbol to look up. |
| `cik` | string | CIK to look up (alternative to `ticker`). |
...
| `search` | string | - | Free-text search over name, sector, industry, or ticker (≤128 chars). |
| `limit` | integer | 100 | Page size (max 5000). |
