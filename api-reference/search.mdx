...
| `tickers` | string[] | all | Restrict the search universe. |
| `period` | string | `ttm` | Metric period. |
| `limit` | integer | 100 | Max results. |
| `filters` | object | `{}` | Map of metric → comparator. Comparators: `gt`, `gte`, `lt`, `lte`, `eq`. |
...
| --- | --- | --- | --- |
| `scenario_id` | string | - | **Required.** |
| `tickers` | string[] | all | Restrict the universe. |
| `period` | string | `quarterly` | Statement period. |
| `limit` | integer | 100 | Max rows. |
| `line_items` | string[] | - | Line items to extract, e.g. `["revenue", "net_income"]`. |
