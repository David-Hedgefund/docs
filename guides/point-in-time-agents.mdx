...
```bash
curl -s "https://api.davidhf.com/scenarios/<id>/manifest" -H "X-API-KEY: YOUR_API_KEY"
```

The manifest gives you `date_range`, `tickers`, and the `time_basis` (including `path_mode`) so you know the clock you're stepping through.
